...
## Стабильная линия и ветка inbox

- **SeriesToolkit 0.1.27** — текущая зафиксированная стабильная версия инструмента нормализации сериалов/мультсериалов.
- Расширенная **входная сортировка** (inbox, Blu-ray, кириллица TMDB и т.д.) вынесена в каталог **`MediaInboxToolkit/`** в корне репозитория; разработка — в ветке **`media-inbox-toolkit`**. Общий модуль метаданных остаётся в **`Fetch-VideoMetadata.ps1`** (корень репозитория).

## 0.1.27 - 2026-04-30 17:05:00 +03:00
- Документация: добавлен операторский раздел в `README.md` по разбору `*-review-queue.csv` (приоритеты, трактовка статусов, практический workflow).
- Документация: добавлены рекомендуемые пороги `rename_min_confidence_apply` для консервативного/сбалансированного/агрессивного режима.
- Публикация: подготовлен и выпущен обновлённый релиз с актуальной инструкцией.
