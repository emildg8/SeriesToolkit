...
if ([string]::IsNullOrWhiteSpace($ToolkitRoot)) {
    if (-not [string]::IsNullOrWhiteSpace($PSScriptRoot)) {
        ...
        if (-not [string]::IsNullOrWhiteSpace($p)) {
            $ToolkitRoot = Split-Path -Parent $p
        }
    }
}
if ([string]::IsNullOrWhiteSpace($ToolkitRoot)) {
    try {
        ...
        if (-not [string]::IsNullOrWhiteSpace($exe)) { $ToolkitRoot = Split-Path -Parent $exe }
    } catch { }
}
if ([string]::IsNullOrWhiteSpace($ToolkitRoot)) {
    throw 'ToolkitRoot пуст: не удалось определить каталог SeriesToolkit (запуск из .exe без пути).'
}
