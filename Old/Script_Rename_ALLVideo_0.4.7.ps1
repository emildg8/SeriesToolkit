...
param(
    ...
    # Имена подпапок с номером сезона: «.S01.», «( S01 )», либо через пробел как в релизах «… S01 …» (например Supernatural S01 BDRemux …), либо «1 сезон», «2 сезон», либо целиком «Season_01», «Season 08» — переименуются в «Сезон 1», «Сезон 2». Папки «Сезон N» шаблону не соответствуют — переименование каталогов для них не выполняется (шаг фактически пропущен). Свой regex: группа (?<sn>…) или первая () с номером сезона.
...
function Measure-UiTextHeight([string]$Text, [System.Drawing.Font]$Font, [int]$MaxWidth) {
    ...
    # TextRenderer.MeasureText(..., WordBreak) часто игнорирует переводы строк — высота получается как у одной строки,
    # из-за этого Multiline TextBox показывает текст «склеенным». Считаем по строкам.
    ...
    foreach ($line in $lines) {
        if ($line.Length -eq 0) {
            $total += [int][Math]::Ceiling($Font.GetHeight())
            continue
        }
        $sz = [System.Windows.Forms.TextRenderer]::MeasureText($line, $Font, [System.Drawing.Size]::new([Math]::Max(80, $MaxWidth), 8000), $flags)
        $total += $sz.Height
    }
    return [Math]::Max($total + 6, 24)
}

function Normalize-UiMultilineText([string]$Text) {
    if ($null -eq $Text) { return '' }
    return ($Text -replace "`r`n", "`n") -replace "`n", [Environment]::NewLine
}

function Format-UiPathForDisplay([string]$Path) {
    if ([string]::IsNullOrWhiteSpace($Path)) { return $Path }
    $p = $Path.Trim()
    if ($p.Length -le 80) { return $p }
    $parts = $p.Split([char]'\')
    if ($parts.Length -le 2) { return $p }
    $sb = [System.Text.StringBuilder]::new()
    if ($parts.Length -ge 3 -and [string]::IsNullOrEmpty($parts[0]) -and [string]::IsNullOrEmpty($parts[1])) {
        [void]$sb.Append('\\' + $parts[2])
        $start = 3
    } else {
    ...
    for ($i = $start; $i -lt $parts.Length; $i++) {
        ...
        [void]$sb.Append([Environment]::NewLine + '\').Append($parts[$i])
    }
    return $sb.ToString()
}
...
function Get-EpisodeNumbersFromVideoFilename([string]$fileName) {
    ...
    # «.S01E.01.» — точка между E и номером эпизода (релизы вида ...S01E.01...)
    ...
    # «1x01», «12x24» — сезон×эпизод (часто без S/E); не путать с 1920x1080
    if ($fileName -match '(?i)(?<![\d])(\d{1,2})x(\d{2,4})(?:-(\d{2,4}))?(?!\d)') {
        $fe = [int]$Matches[2]
        $feEnd = if ($Matches[3]) { [int]$Matches[3] } else { $fe }
        $resLike = @(480, 540, 720, 1080, 1440, 2160, 4320)
        if ($fe -notin $resLike -and $feEnd -notin $resLike -and $fe -le 400 -and $feEnd -le 400) {
            return @{
                Fs    = [int]$Matches[1]
                Fe    = $fe
                FeEnd = $feEnd
...
function Get-FallbackEpisodeTitleFromFilename([string]$fileName) {
    ...
    elseif ($base -match '(?i)[._\s-](\d{1,2})x(\d{2,4})(?:-(\d{2,4}))?[._\s-](.+)$') {
        $epN = [int]$Matches[2]
        if ($epN -notin @(480, 540, 720, 1080, 1440, 2160, 4320) -and $epN -le 400) {
            $chunk = $Matches[4]
        }
    }
...
# Только имя папки целиком: «Season_01», «Season 08», «Season 1» (релизы вроде Игра престолов)
function Get-SeasonEnglishWordFolderRegex {
    return '^(?i)Season[_\s]+(\d+)$'
}
...
function Resolve-SeasonNumberFromFolderName([string]$name) {
    ...
    $se = Get-SeasonEnglishWordFolderRegex
    if ($n -match $se) { return [int]$Matches[1] }
    return $null
}
...
function Set-AppleReadOnlyTextStyle([System.Windows.Forms.TextBox]$tb) {
    # Единый «плоский» вид с фоном формы (как в современных диалогах), без белой рамки.
    $tb.BackColor = [System.Drawing.Color]::FromArgb(248, 248, 250)
...
function Show-WikipediaSearchForm {
    ...
    $tbMsg.Text = (Normalize-UiMultilineText $t.WikiSearchInstructions)
    $tbMsg.Left = $m
    $tbMsg.Top = $m
    $tbMsg.TabIndex = 10
    ...
    $relayoutWiki = {
        $t = Get-UiStrings
        $wikiNorm = (Normalize-UiMultilineText $t.WikiSearchInstructions)
        $tbMsg.Text = $wikiNorm
        $cw = [Math]::Max(400, [int]$form.ClientSize.Width)
        $inner = $cw - 2 * $m
        $tbMsg.Width = $inner
        $tbMsg.Height = [Math]::Max(60, (Measure-UiTextHeight $wikiNorm $tbMsg.Font $inner))
...
function Show-EpisodeSourceUrlForm {
    ...
    $relayoutEp = {
        ...
        $epNorm = (Normalize-UiMultilineText $t.EpisodeInstructions)
        $tbMsg.Text = $epNorm
        ...
        $tbMsg.Top = $m
        $tbMsg.Width = $inner
        $tbMsg.Height = [Math]::Max(72, (Measure-UiTextHeight $epNorm $tbMsg.Font $inner))
        $lblUrl.Left = $m
        ...
        $cbLang.Left = $lblLang.Right + $gapUrlBrowse
        $cbLang.Top = $langTop
        $cbLang.Width = ($m + $inner) - $cbLang.Left
...
function Show-BilingualDialog {
    ...
    [System.Windows.Forms.Application]::EnableVisualStyles()
    $t = Get-UiStrings
    $textNorm = (Normalize-UiMultilineText $text)
    $form = New-Object System.Windows.Forms.Form
    $form.Text = $title
    $form.FormBorderStyle = [System.Windows.Forms.FormBorderStyle]::FixedDialog
    $form.StartPosition = [System.Windows.Forms.FormStartPosition]::CenterScreen
    ...
    $relayoutBd = {
        ...
        $hText = [Math]::Max(40, (Measure-UiTextHeight $textNorm $tb.Font $tb.Width))
        if ($hText -gt 520) {
...
function Show-RenameOutcomeDialog([string]$SeriesBase) {
    ...
    $lblQ.Text = (Normalize-UiMultilineText $t.OutcomeDoneQuestion)
    ...
    $lblCb.Text = (Normalize-UiMultilineText $t.OutcomeCleanupCheckbox)
    ...
    $relayout = {
        $ti = Get-UiStrings
        $qN = (Normalize-UiMultilineText $ti.OutcomeDoneQuestion)
        $cbN = (Normalize-UiMultilineText $ti.OutcomeCleanupCheckbox)
        $lblQ.Text = $qN
        $lblCb.Text = $cbN
        $minClientW = $m + (4 * $dl.BtnColW) + (3 * $dl.Gap) + $m
        $cw = [Math]::Max($minClientW, [int]$form.ClientSize.Width)
        $lblQ.Width = $cw - 2 * $m
        $lblQ.Height = [Math]::Max(28, (Measure-UiTextHeight $qN $lblQ.Font $lblQ.Width))
        $lblCb.Top = $lblQ.Bottom + 10
        $lblCb.Width = $cw - 2 * $m - 24 - $dl.Gap - 8
        $lblCb.Height = [Math]::Max(28, (Measure-UiTextHeight $cbN $lblCb.Font $lblCb.Width))
...
function New-EpisodeTitlesTemplateFile([string]$LiteralPath) {
    $rows = @(
        [pscustomobject]@{ season = 1; episode = 1; title = 'Название эпизода; в имени файла: S01E01, S01.E01, .S01E.01. или 1x01…1x24 (замените на своё)' }
...
if ($Manual) {
    ...
    if (-not $TitlesCsv -or -not (Test-Path -LiteralPath $TitlesCsv)) {
        ...
        $TitlesCsv = $outCsv
        $savedMsg = ($t.SavedBodyFmt -f (Format-UiPathForDisplay $outCsv))
...
# Два шага: (1) подпапки: SeasonFolderMatchRegex («.S01.», …), «N сезон», либо «Season_01» / «Season 8» → «Сезон N»; если все папки уже «Сезон N», для шага .S01/релиз — в лог пишется SKIP. (2) внутри «Сезон N» — файлы с тегом S01E01, «.S1.E01.», «.S01E.01.» или «1x01» (см. Get-EpisodeNumbersFromVideoFilename) → «Сериал - S01E01 - название».
$seasonFolderRe = Get-SeasonFolderRegex
$numericWordSeasonRe = Get-NumericWordSeasonFolderRegex
$seasonEnglishRe = Get-SeasonEnglishWordFolderRegex
...
if (-not $SkipFolderRename) {
    ...
    $seasonEnglishFolders = @(Get-ChildItem -LiteralPath $Base -Directory -ErrorAction SilentlyContinue | Where-Object { $_.Name -match $seasonEnglishRe })
    foreach ($dir in $seasonEnglishFolders) {
        $sn = Resolve-SeasonNumberFromFolderName $dir.Name
        if (-not $sn) { continue }
        $newName = Get-PrefixSeasonFolder $sn
        if ($dir.Name -eq $newName) { continue }
...
function Write-RenameDiagnosticsIfNothingDone {
    ...
    $seasonEngRe = Get-SeasonEnglishWordFolderRegex
    ...
    $pendingSeasonEnglish = @($allDirs | Where-Object { $_.Name -match $seasonEngRe })
    if ($seasonDirs.Count -eq 0) {
        $msgRu = @'
[RU] Переименование не выполнялось: нет подпапок «Сезон 1», «Сезон 2», …
Скрипт: (1) папки с «.S01.», «( S01 )», «… S01 …» (релиз), либо «1 сезон», «2 сезон», либо «Season_01» / «Season 8» → «Сезон 1»; если папки уже «Сезон N», шаг для .S01/релиза только пропускается; (2) внутри «Сезон N» — файлы с S01E01, .S1.E01., .S01E.01. или 1x01 в имени.
Положите видео в «Сезон N»; в имени файла — тег S01E01, .S1.E01., .S01E.01. или 1x01…1x24 (S02E03 и т.д.).
'@
        $msgEn = @'
[EN] Nothing renamed: no «Сезон 1», «Сезон 2», … subfolders.
Script: (1) folders with «.S01.», «( S01 )», release «… S01 …», «1 сезон» / «2 сезон», or «Season_01» / «Season 8» → «Сезон N»; if already «Сезон N», the release step is skipped; (2) inside «Сезон N» — files with S01E01, «.S1.E01.», «.S01E.01.», or «1x01» in the filename.
Put videos in season folders; filenames must include S01E01, dotted S.E tags, or NxNN (1x01, 12x24, …).
'@
        ...
        if ($pendingSeasonEnglish.Count -gt 0) {
            $msgRu += "`n[RU] Есть папки «Season_01» / «Season 8», но «Сезон N» ещё не получено — проверьте конфликт имён или запуск с -SkipFolderRename."
            $msgEn += "`n[EN] Folders named like «Season_01» / «Season 8» exist, but not renamed to «Сезон N» — check name conflicts or -SkipFolderRename."
        }
    ...
    foreach ($d in $seasonDirs) {
        foreach ($f in (Get-ChildItem -LiteralPath $d.FullName -File -Force -ErrorAction SilentlyContinue)) {
            if ($f.Name -match '(?i)S\d+\.E\d+|S\d+E\.\d+|S\d+E\d+|\d{1,2}x\d{2,4}') {
                $anyTag = $true
                break
            }
        }
    ...
    if (-not $anyTag) {
        $msgRu = '[RU] В подпапках «Сезон N» нет файлов с тегом S01E01, .S1.E01., .S01E.01. или 1x01 в имени — нечего переименовывать.'
        $msgEn = '[EN] No files with SxxEyy / .Sxx.Eyy. / .SxxE.yy. / NxNN (1x01) tags in the name inside season folders — nothing to rename.'
        Write-Host $msgRu -ForegroundColor Yellow
        Write-Host $msgEn -ForegroundColor Yellow
...
if (-not $SkipOutcomeDialog) {
    try {
        ...
        elseif ($act -eq 'No') {
            ...
            if ($bundlePath) {
                $tdb = Get-UiStrings
                Show-MessageBox ($tdb.DebugBundleOkFmt -f (Format-UiPathForDisplay $bundlePath)) $tdb.DebugBundleTitle
            } else {
                $tdb = Get-UiStrings
