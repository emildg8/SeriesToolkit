...
function New-EpisodeTitlesTemplateFile([string]$LiteralPath) {
    $rows = @(
        [pscustomobject]@{ season = 1; episode = 1; title = 'Название эпизода; в имени файла допускается S01E01, S01.E01 или .S01E.01. (замените на своё)' }
...
# Два шага: (1) подпапки: SeasonFolderMatchRegex («.S01.», …), «N сезон», либо «Season_01» / «Season 8» → «Сезон N»; если все папки уже «Сезон N», для шага .S01/релиз — в лог пишется SKIP. (2) внутри «Сезон N» — файлы с тегом S01E01, «.S1.E01.» или «.S01E.01.» (см. Get-EpisodeNumbersFromVideoFilename) → «Сериал - S01E01 - название».
...
Скрипт: (1) папки с «.S01.», «( S01 )», «… S01 …» (релиз), либо «1 сезон», «2 сезон», либо «Season_01» / «Season 8» → «Сезон 1»; если папки уже «Сезон N», шаг для .S01/релиза только пропускается; (2) внутри «Сезон N» — файлы с S01E01, .S1.E01. или .S01E.01. в имени.
Положите видео в «Сезон N»; в имени файла — тег S01E01, .S1.E01. или .S01E.01. (S02E03 и т.д.).
'@
        $msgEn = @'
[EN] Nothing renamed: no «Сезон 1», «Сезон 2», … subfolders.
Script: (1) folders with «.S01.», «( S01 )», release «… S01 …», «1 сезон» / «2 сезон», or «Season_01» / «Season 8» → «Сезон N»; if already «Сезон N», the release step is skipped; (2) inside «Сезон N» — files with S01E01, «.S1.E01.», or «.S01E.01.» in the filename.
Put videos in season folders; filenames must include S01E01 or dotted S.E tags (S02E03, .S1.E02., .S01E.02., …).
'@
    ...
    foreach ($d in $seasonDirs) {
        foreach ($f in (Get-ChildItem -LiteralPath $d.FullName -File -Force -ErrorAction SilentlyContinue)) {
            if ($f.Name -match '(?i)S\d+\.E\d+|S\d+E\.\d+|S\d+E\d+') {
                $anyTag = $true
                break
            }
        }
    ...
    if (-not $anyTag) {
        $msgRu = '[RU] В подпапках «Сезон N» нет файлов с тегом S01E01, .S1.E01. или .S01E.01. в имени — нечего переименовывать.'
        $msgEn = '[EN] No files with SxxEyy / .Sxx.Eyy. / .SxxE.yy. tags in the name inside season folders — nothing to rename.'
        Write-Host $msgRu -ForegroundColor Yellow
        Write-Host $msgEn -ForegroundColor Yellow
