...
function New-EpisodeTitlesTemplateFile([string]$LiteralPath) {
    ...
    $rows | Export-Csv -LiteralPath $LiteralPath -Encoding UTF8 -NoTypeInformation
}
...
'@
            ...
            if (-not $items -and -not $wikiUrlFromFirst) {
                ...
                if ($ws.Action -eq 'Cancel') {
                    Show-MessageBox "[RU] Отменено.`n`n[EN] Cancelled." 'Rename-Series'
                    exit 1
                }
                $q = $ws.Text
...
'@ + $extraHint
        ...
        $items | Export-Csv -LiteralPath $outCsv -Encoding UTF8 -NoTypeInformation
        $TitlesCsv = $outCsv
        Show-MessageBox "[RU] Сохранено: $outCsv`nЗапускаю переименование...`n`n[EN] Saved: $outCsv`nStarting rename..." 'Rename-Series'
    }
}
...
Get-ChildItem -LiteralPath $Base -Directory | ForEach-Object {
    ...
    Get-ChildItem -LiteralPath $_.FullName -File -Force | ForEach-Object {
        ...
        $title = $st[$fe]
        if (-not $title) {
            Write-Warning "No title for $tag in $fn"
            return
        }
