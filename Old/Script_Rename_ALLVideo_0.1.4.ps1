...
param(
    ...
    # Имена подпапок с номером сезона: «.S01.» / «.S02.» или «( S01 )» / «(S02)» в имени — будут переименованы в «Сезон 1», «Сезон 2». Папки, уже названные «Сезон N», шаблону не соответствуют — переименование каталогов для них не выполняется (шаг фактически пропущен).
    [string]$SeasonFolderMatchRegex = '(?i)(?:\.|\(\s*)S(\d+)(?:\.|\s*\))',
...
function Export-CsvUtf8BomEpisodeList([object[]]$Objects, [string]$LiteralPath) {
    ...
    if (-not $Objects -or @($Objects).Count -eq 0) {
        [System.IO.File]::WriteAllText($LiteralPath, '', $enc)
        return
    }
    $lines = $Objects | ConvertTo-Csv -NoTypeInformation
    [System.IO.File]::WriteAllLines($LiteralPath, [string[]]$lines, $enc)
}
...
# Два шага: (1) подпапки по шаблону SeasonFolderMatchRegex → «Сезон N»; если все папки уже «Сезон N», переименование каталогов не требуется — в лог пишется SKIP. (2) внутри «Сезон N» — файлы с S01E01 в имени → «Сериал - S01E01 - название».
...
if (-not $SkipFolderRename) {
    ...
    if ($dotSFolders.Count -eq 0) {
        ...
        if ($alreadySeasonDirs.Count -gt 0) {
            Write-RenameLog $Base 'SKIP folder rename: folders already «Сезон N» (no .S01./( S01 )-style names to convert)'
        }
    }
...
function Write-RenameDiagnosticsIfNothingDone {
    $seasonRe = Get-SeasonFolderRegex
    $allDirs = @(Get-ChildItem -LiteralPath $Base -Directory -ErrorAction SilentlyContinue)
    $seasonDirs = @($allDirs | Where-Object { $_.Name -match $seasonRe })
...
Скрипт: (1) папки с «.S01.» или «( S01 )» в имени → «Сезон 1»; если папки уже «Сезон N», этот шаг только пропускается; (2) внутри «Сезон N» — файлы с S01E01 в имени.
Положите видео в «Сезон N»; в имени файла должен быть тег S01E01 (S02E03 и т.д.).
'@
        $msgEn = @'
[EN] Nothing renamed: no «Сезон 1», «Сезон 2», … subfolders.
Script: (1) folders with «.S01.» or «( S01 )» in the name → «Сезон N»; if already «Сезон N», that step is skipped; (2) inside «Сезон N» — files with S01E01 in the filename.
Put videos in season folders; filenames must include S01E01 (S02E03, …).
'@
        if ($pendingDotS.Count -gt 0) {
            $msgRu += "`n[RU] Обнаружены папки по шаблону сезона (.S01., ( S01 ) и т.д.), но имя «Сезон N» ещё не создано — проверьте конфликт имён или запуск с -SkipFolderRename."
            $msgEn += "`n[EN] Folders matching season pattern (.S01., ( S01 ), …) exist, but not «Сезон N» yet — check name conflicts or -SkipFolderRename."
        }
    ...
    if (-not $anyTag) {
        $msgRu = '[RU] В подпапках «Сезон N» нет файлов с тегом S01E01 в имени — нечего переименовывать.'
        $msgEn = '[EN] No files with SxxEyy in the name inside season folders — nothing to rename.'
        Write-Host $msgRu -ForegroundColor Yellow
        Write-Host $msgEn -ForegroundColor Yellow
