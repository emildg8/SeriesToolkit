...
param(
    ...
    # Имена подпапок с номером сезона: «.S01.», «( S01 )», либо через пробел как в релизах «… S01 …» (например Supernatural S01 BDRemux …) — переименуются в «Сезон 1», «Сезон 2». Папки «Сезон N» шаблону не соответствуют — переименование каталогов для них не выполняется (шаг фактически пропущен). Свой regex: группа (?<sn>…) или первая () с номером сезона.
...
Скрипт: (1) папки с «.S01.», «( S01 )» или «… S01 …» (релизное имя) в имени → «Сезон 1»; если папки уже «Сезон N», этот шаг только пропускается; (2) внутри «Сезон N» — файлы с S01E01 в имени.
...
Script: (1) folders with «.S01.», «( S01 )», or release-style «… S01 …» in the name → «Сезон N»; if already «Сезон N», that step is skipped; (2) inside «Сезон N» — files with S01E01 in the filename.
