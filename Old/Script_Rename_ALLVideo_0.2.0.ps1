...
function Normalize-EpisodeTitleForFileName([string]$title, [int]$episodeInSeason) {
    ...
    $t = $title
    # Каталоги вроде TVDB: "English Title |AltTitle = Русское название" — в имени файла только русская часть
    if ($t -match '(?i)\|\s*AltTitle\s*=\s*(.+)$') {
        $t = $Matches[1].Trim()
    }
...
function Show-RenameOutcomeDialog([string]$SeriesBase) {
    ...
    $lbl.Left = 12
    $lbl.Top = 12
    $lbl.Width = $form.ClientSize.Width - 24
    $lbl.Height = 72
    $lbl.Text = @'
[RU] Всё прошло успешно?

[EN] Did everything complete successfully?
'@
    ...
    $cb.Left = 12
    $cb.Top = $lbl.Bottom + 8
    $cb.Width = $form.ClientSize.Width - 24
    $cb.Height = 48
    $cb.Checked = $true
    $cb.Text = @'
[RU] Удалить в папке сериала: episode-titles.csv, rename-series-log.txt, Переименовать-сериал.cmd

[EN] Delete in the series folder: episode-titles.csv, rename-series-log.txt, Переименовать-сериал.cmd
'@
    ...
    $btnNo.Width = $btnW
    $btnNo.Height = $btnH
    $btnNo.Left = $btnCancel.Left - $btnGap - $btnW
    $btnNo.Top = $btnTop
    $btnNo.TabIndex = 3
    $btnNo.Add_Click({
            $form.Tag = @{ Action = 'No' }
    ...
    $btnYes.Width = $btnW
    $btnYes.Height = $btnH
    $btnYes.Left = $btnNo.Left - $btnGap - $btnW
    $btnYes.Top = $btnTop
    $btnYes.TabIndex = 1
    $btnYes.Add_Click({
    ...
    $form.CancelButton = $btnCancel
    $form.Add_Shown({ $form.ActiveControl = $btnYes })
...
# Два шага: (1) подпапки: SeasonFolderMatchRegex («.S01.», …) или «N сезон» → «Сезон N»; если все папки уже «Сезон N», для шага .S01/релиз — в лог пишется SKIP. (2) внутри «Сезон N» — файлы с S01E01 в имени → «Сериал - S01E01 - название».
...
Скрипт: (1) папки с «.S01.», «( S01 )», «… S01 …» (релиз), либо «1 сезон», «2 сезон» → «Сезон 1»; если папки уже «Сезон N», шаг для .S01/релиза только пропускается; (2) внутри «Сезон N» — файлы с S01E01 в имени.
...
Script: (1) folders with «.S01.», «( S01 )», release «… S01 …», or «1 сезон» / «2 сезон» → «Сезон N»; if already «Сезон N», the release step is skipped; (2) inside «Сезон N» — files with S01E01 in the filename.
    ...
    try {
        ...
        elseif ($act -eq 'No') {
            ...
            if ($bundlePath) {
                Show-MessageBox @"
[RU]
Собран архив для отладки:
$bundlePath

Передайте этот ZIP ассистенту Cursor (или приложите к сообщению об ошибке).

[EN]
Debug bundle saved:
$bundlePath

Send this ZIP to the Cursor assistant (or attach it to your bug report).
